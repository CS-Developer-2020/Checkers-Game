class CheckerSoldier():
    ...
    # Description: assign the soldier's neighbors tiles. If a neighbor tile does not exist, either its row or column will equal 100
    ...
    # Description: updates soldier's location. reassigns neighbors, sets self to king if soldier became a king
    def updateLocation(self, row_, column_):
        self.row = row_
        self.column = column_
        self.assignNeighbors()
        if row_ == 0 and self.color == "black":
            if not self.isKing():
                self.setToKing()
        if row_ == 7 and self.color == "white":
            if not self.isKing():
                self.setToKing()
